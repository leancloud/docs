...
* 云引擎需要重新部署。
* 短信签名与模板需要重新审核。
* 推送厂商设置、开发证书上传、敏感词库上传等应用维度的设置信息都需要重新配置。

### 华北节点、华东节点与国际版如何选择，各节点的区别是什么？

如果用户主体在国内选择国内的华北和华东节点，用户主体在国外选择国际版。跨节点访问例如国内访问国际版、或者国外访问国内节点会有网络延迟，所以建议根据用户所在地选择节点。

华北节点和华东节点在国内不同地区的访问速度基本没有差别。目前华北节点的功能更完善一些，有一些功能在华东还不支持，例如，华东节点暂不支持在控制台自助绑定独立 IP、暂时没有 API 访问日志功能、不支持日志表与离线分析、混合推送中不支持 FCM 推送。如果需要使用这些服务可以选择华北节点。
